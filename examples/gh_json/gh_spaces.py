"""
gh_spaces.py — GH Script: "Import IFC Spaces"
==============================================

Reads ``IfcSpace`` entities from an IFC file and draws them in the active
Rhino document as 2-D footprint curves, hatch fills, TextDot annotations
and/or 3-D mesh bodies.
...
hatch_pattern : str   — Rhino hatch pattern for space fills (default "Solid").
import_fp     : bool  — Draw 2-D footprint curves (default True).
import_hatch  : bool  — Draw hatch fills (default True).
import_ann    : bool  — Draw TextDot labels with name + area (default True).
import_mesh   : bool  — Tessellate and draw 3-D bodies (default False).
mesh_quality  : str   — Tessellation quality preset
                         superfine/fine/default/coarse/supercoarse
                         (default "default").
clear         : bool  — Clear existing objects under layer_root first
                         (default True).

Component outputs
-----------------
out : str — Status message (space count, object counts, or error).
"""
